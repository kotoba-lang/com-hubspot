(ns hubspot.main
  "Kotodama WASM entrypoint for the Hubspot clean-room actor (L5) — Clojure port.
  ...
  facts (`hubspot.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "hubspot")
(def tier "L5")
...
(def entity-specs
  [{:entity "Contact"  :plural "contacts"  :id-prefix "hubspot_con"
    :fields [:email :firstname :lastname :company :phone :website] :required [:email :firstname]
    :coerce {}                       :refs {}}
   {:entity "Company"  :plural "companies" :id-prefix "hubspot_com"
    :fields [:name :domain :industry]      :required [:name :domain]
    :coerce {}                       :refs {}}
   {:entity "Deal"     :plural "deals"     :id-prefix "hubspot_dea"
    :fields [:dealname :dealstage :amount :pipeline] :required [:dealname :dealstage]
    :coerce {:amount :float}         :refs {}}
   {:entity "Ticket"   :plural "tickets"   :id-prefix "hubspot_tic"
    :fields [:subject :hsTicketPriority :hsPipelineStage] :required [:subject :hsTicketPriority]
    :coerce {}                       :refs {}}
   {:entity "LineItem" :plural "lineitems" :id-prefix "hubspot_lin"
    :fields [:name :quantity :price]       :required [:name :quantity]
    :coerce {:quantity :int :price :float} :refs {}}
   {:entity "Pipeline" :plural "pipelines" :id-prefix "hubspot_pip"
    :fields [:label :displayOrder]         :required [:label :displayOrder]
    :coerce {:displayOrder :int}     :refs {}}])
...
(defn emit-facts
  "EAVT facts for one record: {\"hubspot.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "hubspot-compat" :tier tier :entities entities} 200])
...
(defn start! [] :hubspot-compat/ready)
